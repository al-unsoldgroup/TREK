...
describe('native Trip Advice guest controller', () => {
  ...
  it('deduplicates metadata requests within the guest session', async () => {
    vi.mocked(publicShareApi.writeV2).mockResolvedValue({ version: 2, kind: 'places.metadata', data: { placeKey: 'p:1', primaryType: 'Garden' } })
    const view = renderHook(() => useTripAdviceSession('ta_abcdefghijklmnopqrstuvwxyz123456'))
    await waitFor(() => expect(view.result.current.projection).toBeTruthy())
    const results = await Promise.all([view.result.current.metadata('p:1'), view.result.current.metadata('p:1')])
