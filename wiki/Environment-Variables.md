...
| `FEED_CACHE_TTL_SECONDS`    | How long a built [calendar feed](Calendar-Feeds) body may be reused before it is rebuilt. The all-trips feed can render an entire account's travel history on a single request, and every subscribed device fetches on its own timer, so caching collapses those into one build. The cost is staleness: an edit can take up to this long to appear in a feed. Calendar apps refresh on the order of an hour, so the default is invisible in practice. Set `0` to disable feed caching. Values above `3600` are capped; invalid values warn at startup and fall back to the default. | `60`                            |
