...
import { ADVICE_PLUGIN_ID, adviceActionSchema, adviceFeedbackReadSchema, adviceFeedbackWriteSchema, advicePlacesAutocompleteResultSchema, advicePlacesResolveResultSchema, advicePhotoResultSchema, idParamSchema } from '@trek/shared';
...
export class PluginSharePublicController {
  ...
  async action(@Param('token') token: string, @Body() body: AdviceActionDto, @Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const action = adviceActionSchema.parse(body);
    this.headers(res);
    this.shares.requireOrigin(req.get('origin'), req.get('sec-fetch-site'));
    const principal = this.shares.authorize(token, this.shares.credential(req.get('cookie')), req.get('x-trek-advice-csrf'));
    ...
    let result: unknown;
    try {
      result = action.kind === 'places.autocomplete'
        ? await (this.places ?? unavailableProvider()).autocomplete(principal, action)
    ...
    const parsed = action.kind === 'read' ? adviceFeedbackReadSchema.safeParse(result)
      : action.kind === 'places.autocomplete' ? advicePlacesAutocompleteResultSchema.safeParse((result as { data?: unknown }).data)
        : action.kind === 'places.resolve' ? advicePlacesResolveResultSchema.safeParse((result as { data?: unknown }).data)
